        ...
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data">
            ...
            @if ($category->image)
                <div class="mb-4">
                    ...
                    <img src="{{ Storage::url($category->image) }}" alt="{{ $category->name }}" class="h-20 w-20 rounded-md object-cover">
                </div>
            @endif
            ...
            <div class="mb-4">
                <label for="image" class="block text-sm font-medium text-gray-700 mb-1">
                    {{ $category->image ? 'Ganti Gambar' : 'Gambar Ikon' }}
                </label>
                <input type="file"
                       name="image"
                       id="image"
                ...
                <p class="mt-1 text-xs text-gray-500">Format: JPG, PNG, WebP. Maksimum 2MB. Kosongkan jika tidak ingin mengubah.</p>
                @error('image')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <div id="image-preview" class="mt-2 hidden">
                    <p class="text-xs text-gray-500 mb-1">Gambar baru:</p>
